...
const (
	...
	Finalizer                   = "capa-operator.finalizers.giantswarm.io/karpenter-controller"
...
func (r *KarpenterMachinePoolReconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	// We only remove the finalizer when there are no nodeclaims in the workload cluster, because we want to block cluster deletion until karpenter had enough time to clean up.
	if !karpenterMachinePool.GetDeletionTimestamp().IsZero() {
		...
		if err != nil {
			return reconcile.Result{}, fmt.Errorf("couldn't create remote workload cluster client: %w", err)
		}

		nodeClaimList, err := getWorkloadClusterNodeClaims(ctx, workloadClusterClient)
		if err != nil {
			return reconcile.Result{}, fmt.Errorf("couldn't list nodeclaims in workload cluster: %w", err)
		}

		if len(nodeClaimList.Items) > 0 {
			logger.Info("NodeClaims still exist in workload cluster, requeueing", "numberOfNodeClaims", len(nodeClaimList.Items))
			return reconcile.Result{RequeueAfter: 30 * time.Second}, nil
		}

		controllerutil.RemoveFinalizer(karpenterMachinePool, Finalizer)
	...
	updated := controllerutil.AddFinalizer(karpenterMachinePool, Finalizer)
	if updated {
		if err := r.client.Patch(ctx, karpenterMachinePool, client.MergeFrom(karpenterMachinePoolCopy)); err != nil {
